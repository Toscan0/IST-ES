...
public class ActivityInterface {
	...
	@Atomic(mode = TxMode.READ)
	public static ActivityOfferData getActivityOfferDataByCode(String codeOffer) {
		ActivityOffer offer = getOfferByCode(codeOffer);
	...
	@Atomic(mode = TxMode.READ)
	private static ActivityOffer getOfferByCode(String codeOffer) {
		return FenixFramework.getDomainObject(codeOffer);
	}
	
	@Atomic(mode = TxMode.WRITE)
	public static void createBooking(String codeProvider, String codeActivity, String reference, ActivityReservationData reservation) {
		ActivityProvider activityProvider = getProviderByCode(codeProvider);
		...
		new Booking(activityProvider, activityOffer, reservation.getBuyerNif(), reservation.getBuyerIban());
	}
	@Atomic(mode = TxMode.WRITE)
	...
	private static ActivityOffer getActivityOfferByReference(String codeProvider, String codeActivity,
			String reference) {
		Activity activity = getActivityByCode(codeProvider, codeActivity);
		...
		Set<ActivityOffer> activityOffer = activity.getActivityOfferSet();

		if(activityOffer == null)
			return null;

		ActivityOffer activityOfferResult = null;

		for(ActivityOffer activityOfferN : activityOffer){
			if(activityOfferN.getReference().equals(reference)){
				activityOfferResult = activityOfferN;
			}
		}

		return activityOfferResult;
	}
}
